...
export const label = embed({
    ...
    commands: editor => (label, color) => editor.insert({ label, color }),
    render: (tag) => {
...
const tsReplacements = [
    ...
    [/(@\()[\d.:-]*\).$/s, (capture) => {
            if (capture.includes('-')) {
                ...
                return { ts: `@(${parseTimes(capture.slice(2, -1))})` };
        }]
];
...
export const tsReplace = (editor, index, prefix) => {
    return tsReplacements.some(([regexp, getAttributes]) => {
        ...
        if (match) {
            ...
            if (attributes.ts !== text) {
                editor.insert(attributes.ts, attributes, [end - text.length, end]);
            }
            else {
